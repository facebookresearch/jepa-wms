...
# Taken from https://github.com/jy0205/Pyramid-Flow
...
class LPIPS(nn.Module):
    ...
    def __init__(self, use_dropout=True, lpips_ckpt_path=None):
        super().__init__()
        if lpips_ckpt_path is None:
            pretrained_ckpt_root = os.environ.get("PRETRAINED_CKPT_ROOT")
            lpips_ckpt_path = f"{pretrained_ckpt_root}/vgg_lpips.pth"
    ...
    def forward(self, input, target, in01=True):
        """
        Input is (N,3,H,W), range [-1,+1]
        """
        if in01:  # input and target in [0,1]
            input = (input - 0.5) * 2
            target = (target - 0.5) * 2
...
class vgg16(torch.nn.Module):
    def __init__(self, requires_grad=False, pretrained=True):
        ...
        vgg_pretrained_features = models.vgg16(weights=models.VGG16_Weights.DEFAULT if pretrained else None).features
