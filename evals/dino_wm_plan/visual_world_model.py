...
class VWorldModel(nn.Module):
    def __init__(
        ...
        self.tubelet_size_orig = self.encoder.tubelet_size_enc if hasattr(encoder, "tubelet_size_enc") else 1
        self.tubelet_size = tubelet_size
        self.tubelet_size_act = tubelet_size_act
        self.dup_image = dup_image
        if self.dup_image: self.tubelet_size, self.tubelet_size_act = 1, 1
        self.grid_num_hist = self.num_hist // self.tubelet_size
        self.grid_num_pred = self.num_pred // self.tubelet_size
    ...
    def encode_obs(self, obs):
        ...
        if self.dup_image:
            visual = visual.unsqueeze(2).repeat(1, 1, self.tubelet_size_orig, 1, 1)
        if self.causal_enc and self.encoder.name in ["vjepa", "vjepa2"]:
            visual = rearrange(visual, "(b t) c h w -> b c t h w", b=b)
    ...
    def decode_obs(self, z_obs):
        ...
        visual = rearrange(visual, "(b t) c h w -> b t c h w", t=num_frames * self.tubelet_size)
        obs = {
            "visual": visual,
    ...
    def forward(self, obs, act):
        ...
        z_src = z[:, :self.grid_num_hist, :, :]  # (b, num_hist, num_patches, dim)
        z_tgt = z[:, self.grid_num_pred:, :, :]  # (b, num_hist, num_patches, dim)
        # Default param is num_pred=1, so tgt is of time dim num_hist
        # num_pred is the number of frames on which NOT to compute loss
        # if num_hist=4, num_pred=2, predict 4 last frames from first 4 frames
        visual_src = obs['visual'][:, : self.num_hist, ...]  # (b, num_hist, 3, img_size, img_size)
        visual_tgt = obs['visual'][:, self.num_pred :, ...]  # (b, num_hist, 3, img_size, img_size)

        if self.predictor is not None:
            z_pred = self.predict(z_src) # (b, grid_num_hist, num_patches, dim)
            if self.decoder is not None:
    ...
    def rollout(self, obs_0, act, step_size=1):
        ...
        """
        # assert step_size == 1, f"step_size is {step_size}, currently only support step_size = 1."
        # inc and step_size is in frames units. But all that regards z is in grid_size units.
        assert (
            step_size <= self.num_pred
        ), f"step_size {step_size} should <= num_pred {self.num_pred}"
        ...
        while t < action.shape[1]:
            # z is updated in the loop
            z_pred = self.predict(z[:, -self.grid_num_hist :])
            z_new = z_pred[:, -1:, ...]
            # replace action with the current action
            z_new = self.replace_actions_from_z(z_new, action[:, t : t + inc, :])
            z = torch.cat([z, z_new], dim=1) # shift the embeddings context by adding new pred
            t += inc
        z_pred = self.predict(z[:, -self.grid_num_hist :])
