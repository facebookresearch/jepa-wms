...
class PlanEvaluator:  # evaluator for planning
    ...
    def _get_trajdict_last(self, dct, length):
        # length: length of final slice to take
        new_dct = {}
    ...
    def _get_traj_last(self, traj_data, length):
        ...
        if isinstance(traj_data, torch.Tensor):
            traj_data = traj_data[:, slice_start:]
        else:
            traj_data = traj_data[:, slice_start:]
        return traj_data
    # def _get_traj_last(self, traj_data, length):
    ...
    def eval_actions(
        ...
        i_final_z_obs = self._get_trajdict_last(i_z_obses, 1)
        # Careful: trans_obs_0 are adjacent. But with training with tubelet_size>1,
        # model predicts from a tubelet_size actions at tubelet_size * frameskip
        # So there should not be padding between 2 first frames as they are adjacent
        ...
        e_final_obs = self._get_trajdict_last(e_obses, self.wm.tubelet_size)
        e_final_state = self._get_traj_last(e_states, 1)[
            :, 0
        ]  # reduce dim back
        ...
        if self.wm.decoder is not None:
            ...
            # discard tubelet_size decoded frame as it was adjacent to first decoded frame
            if self.wm.tubelet_size > 1:
                i_visuals = torch.cat((i_visuals[:, :self.wm.tubelet_size, :, :, :],
                                i_visuals[:, self.wm.tubelet_size+1:, :, :, :]), dim=1)
    ...
    def _compute_rollout_metrics(self, e_state, e_obs, i_z_obs):
        """
        Compute distance between last imagined state and embedding of last gt state
        Args
            e_state
            e_obs
            i_z_obs
    ...
    def _plot_rollout_compare(
        ...
        """
        decoding of imagined visuals i_visuals may have less frames than e_visuals due to frameskip, so pad accordingly
        e_visuals: (b, t, h, w, c)
        i_visuals: (b, t, h, w, c)
        goal: (b, h, w, c)
        """
